...
from inquisitor.translators import cisco_ios
...
def worker(host, command_set):
    ...
    with CiscoDriver(host, config['username'], config['password'],
        ...
        host_data['interfaces'] = cisco_ios.show_interfaces_to_dict(result[1])
        host_data.update(cisco_ios.show_version_to_dict(result[0]))
        host_data['ipv4_protocols'] = cisco_ios.show_ip_protocols_summary_to_dict(result[3])
        host_data['storage_volumes'] = cisco_ios.dir_to_dict(result[4])
        host_data['routes'] = cisco_ios.show_ip_route_static_to_dict(result[5])
        host_data['arp_table'] = cisco_ios.show_ip_arp_to_dict(result[6])

        vrf_dict = cisco_ios.show_vrf_to_dict(result[2])
        ...
        for vrf in vrf_dict.keys():
            ...
            host_data['vrfs'][vrf]['ipv4_protocols'] = cisco_ios.show_ip_protocols_summary_to_dict(vrf_result[0])
        ...
        for vrf in vrf_dict.keys():
            ...
            host_data['vrfs'][vrf]['arp_table'] = cisco_ios.show_ip_arp_to_dict(vrf_result[0])
